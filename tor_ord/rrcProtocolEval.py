...
for i in list(range(0, len(stims))):
    mod_test, proto_test = get_ind_data(data_type)

    mod_test.set_state(state) #use state after prepacing
    proto_test.schedule(5.3, 0.2, 1, 1000, 0)
    proto_test.schedule(stims[i], 4004, 995, 1000, 1)
    sim_test = myokit.Simulation(mod_test, proto_test)
    dat_test = sim_test.run(6000)

    t, v, cai, i_ion = get_last_ap(dat_test, 4)
    plt.plot(t, v, label = "stimulus = " + str(-stims[i])+ " A/F")
plt.legend()

#plt.plot(dat_test["engine.time"], dat_test["membrane.v"])

# %% testing RRC without protocol - exact
#data_type = baseline
#state = IC
data_type = optimized
state = IC1
RRC = np.array(0.1) #set RRC

stims = [0, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2, 0.225, 0.25, 0.275, 0.3]
start = np.where(stims == RRC)[0][0]
stims_narrow = np.linspace(stims[start], stims[start+1], 11)
...
for i in list(range(0, len(stims))):
    ...
    mod_test.set_state(state) #use state after prepacing
    proto_test.schedule(5.3, 0.2, 1, 1000, 0)
    proto_test.schedule(stims_narrow[i], 4004, 995, 1000, 1)
    sim_test = myokit.Simulation(mod_test, proto_test)
    dat_test = sim_test.run(6000)
